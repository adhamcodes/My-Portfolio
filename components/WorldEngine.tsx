...
function ZeroUploadWorld() {
  const steps = ["YOUR DEVICE", "THE BROWSER", "PROCESS LOCALLY", "SHARE WHEN NEEDED"];
  ...
  const run = async () => {
    ...
    for (let index = 0; index < steps.length; index += 1) {
      setStep(index);
      window.dispatchEvent(new CustomEvent("aura:signal", { detail: `ZEROUPLOAD · ${steps[index]}` }));
  ...
  return (
    <div className="world-engine world-engine-zero">
      <div className="engine-head"><span>WHAT STAYS ON YOUR DEVICE</span><b>LOCAL-FIRST MODEL</b></div>
      ...
      <div className="zero-readout"><span>DESIGN RULE</span><b>KEEP USEFUL WORK IN THE BROWSER.</b><em>Only send data away when the product actually needs to.</em></div>
      <button className="engine-action" onClick={run} disabled={running} data-cursor="signal">{running ? "RUNNING…" : "PLAY THE FLOW"} ↗</button>
    </div>
  );
}
...
function WindowBiomeWorld() {
  ...
  return (
    <div className="world-engine world-engine-biome">
      <div className="engine-head"><span>A DESKTOP THAT REACTS TO FOCUS</span><b>CLICK A WINDOW</b></div>
      <div className="biome-desktop">
        ...
        <motion.div className="biome-organism" animate={{ x: active === "code" ? -110 : active === "terminal" ? -30 : 110, y: active === "terminal" ? 80 : active === "code" ? -20 : 10, rotate: active === "code" ? -28 : active === "terminal" ? 24 : 9 }} transition={{ type: "spring", stiffness: 75, damping: 14 }}>
          <i /><i /><i /><b>OVERLAY / {active.toUpperCase()}</b>
        </motion.div>
      </div>
...
function NovaWorld() {
  ...
  return (
    <div className="world-engine world-engine-nova" style={{ "--nova-energy": energy / 100 } as React.CSSProperties}>
      <div className="engine-head"><span>A MOTION STUDY</span><b>TRY THE CONTROLS</b></div>
      <div className={`nova-stage nova-${mode}`}>
        <div className="nova-starfield" /><div className="nova-orb"><i /><i /><i /></div>
        <div className="nova-title"><span>VISUAL STUDY</span><b>NOVA</b><em>{mode.toUpperCase()} · {energy}%</em></div>
      </div>
